    ...
    <div style={{ display: "flex", height: "100vh", fontFamily: b, background: "#FAF8F5", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <div style={{ background: "white", borderBottom: "1px solid #F0ECE6", height: 60, padding: "0 32px", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
          <div>
            <div style={{ fontSize: 22, fontWeight: 400, color: "#022350", fontFamily: h }}>Lernpfade</div>
              ...
              <div key={path.id}>
                {i > 0 && (
                  <div style={{ display: "flex", justifyContent: "center", marginBottom: 16 }}>
                    <div style={{ width: 2, height: 24, background: "#F0ECE6" }} />
                  </div>
                )}
                <div style={{ background: "white", borderRadius: 16, border: path.status === "active" ? "2px solid #0FA4A0" : "1px solid #F0ECE6", padding: "24px 28px", opacity: path.status === "locked" ? 0.6 : 1, position: "relative", overflow: "hidden" }}>
                  {path.status === "done" && (
                  ...
                  {path.status === "locked" && (
                    <div style={{ position: "absolute", top: 16, right: 16, background: "#FAF8F5", color: "#9A9AAA", fontSize: 11, fontWeight: 700, padding: "3px 10px", borderRadius: 20 }}>🔒 Gesperrt</div>
                  )}
                  ...
                  <div style={{ display: "flex", gap: 16, alignItems: "flex-start" }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      {path.status !== "locked" && (
                        <a href="/courses" style={{ display: "inline-block", marginTop: 14, padding: "8px 20px", background: path.status === "done" ? "#FAF8F5" : path.color, color: path.status === "done" ? "#4A4A5A" : "white", borderRadius: 9, fontSize: 13, fontWeight: 500, textDecoration: "none" }}>
