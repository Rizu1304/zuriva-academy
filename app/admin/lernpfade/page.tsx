...
import {
  ...
  Bell, BookOpenCheck, FileText, Lightbulb,
} from "lucide-react";
...
const STEP_TYPES: Record<string, { label: string; Icon: typeof BookOpenCheck; color: string }> = {
  course: { label: "Kurs", Icon: BookOpenCheck, color: "#022350" },
  exam: { label: "Prüfung", Icon: FileText, color: "#C8A24D" },
  activity: { label: "Aktivität", Icon: Lightbulb, color: "#0FA4A0" },
};
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={{ padding: "20px 36px", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            {view === "list" ? (
              <button onClick={openCreate} style={{ padding: "8px 16px", background: marine, color: "white", borderRadius: 12, fontSize: 12.5, fontWeight: 500, border: "none", cursor: "pointer", fontFamily: b }}>+ Neuer Lernpfad</button>
            ) : (
            ...
            <>
              ...
              {filtered.length === 0 ? (
                <div style={{ ...glassCard, padding: "60px 24px", textAlign: "center" }}>
                  <div style={{ width: 48, height: 48, borderRadius: 16, background: "rgba(2,35,80,0.06)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 12px", color: marine }}><Map size={24} /></div>
                  <div style={{ fontSize: 16, fontWeight: 500, color: marine, fontFamily: h, marginBottom: 4 }}>Keine Lernpfade gefunden</div>
                  <div style={{ fontSize: 13, color: "#8090A0", marginBottom: 16 }}>Erstellen Sie Ihren ersten Lernpfad.</div>
                  <button onClick={openCreate} style={{ padding: "8px 20px", background: marine, color: "white", borderRadius: 12, fontSize: 12.5, fontWeight: 500, border: "none", cursor: "pointer", fontFamily: b }}>+ Neuer Lernpfad</button>
                </div>
              ) : (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
              ...
              <div style={{ ...glassCard, padding: "28px 28px" }}>
                ...
                <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
                  ...
                  <button onClick={savePath} disabled={!form.title.trim()} style={{ padding: "10px 24px", borderRadius: 12, border: "none", fontSize: 13, fontWeight: 500, cursor: "pointer", background: !form.title.trim() ? "rgba(0,0,0,0.06)" : marine, color: !form.title.trim() ? "#9A9AAA" : "white", fontFamily: b }}>
              ...
              <div style={{ ...glassCard, padding: "28px 28px", display: "flex", flexDirection: "column" }}>
                        ...
                        return (
                          <div key={step.id} style={{ padding: "14px 18px", borderRadius: 14, border: "1px solid rgba(0,0,0,0.04)", background: "rgba(0,0,0,0.02)" }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                              <span style={{ width: 28, height: 28, borderRadius: 12, background: `linear-gradient(135deg, ${st.color}, ${st.color}cc)`, color: "white", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, flexShrink: 0 }}><st.Icon size={14} /></span>
                              <div style={{ flex: 1 }}>
                ...
                <div style={{ borderTop: "1px solid rgba(0,0,0,0.04)", paddingTop: 16 }}>
                  ...
                  <button onClick={addStep} disabled={!newStep.title.trim()} style={{ padding: "8px 16px", borderRadius: 12, border: "none", fontSize: 12.5, fontWeight: 500, cursor: "pointer", background: newStep.title.trim() ? "#0FA4A0" : "rgba(0,0,0,0.06)", color: newStep.title.trim() ? "white" : "#9A9AAA", width: "100%", fontFamily: b }}>+ Schritt hinzufügen</button>
                </div>
              </div>
            </div>
          )}
