...
const navItems = [
  ...
  { name: "Kurse", active: false },
  { name: "Lernpfade", active: false },
  { name: "Prüfungen", active: false },
  { name: "Zertifikate", active: false },
...
const stats = [
  ...
  { n: "342 Pts", l: "Credits total", t: "↑ +24 diese Woche", red: false },
  { n: "34 h", l: "Lernzeit total", t: "↑ +3.2h diese Woche", red: false },
...
const courses = [
  ...
  { img: "https://images.unsplash.com/photo-1521791136064-7986c2920216?w=600&q=80", tag: "Faehigkeiten", title: "Beratungskompetenz", p: 85 },
];

export default function Dashboard() {
  const [chatOpen, setChatOpen] = useState(false);
  const [messages, setMessages] = useState([{ role: "bot", text: "Hallo! Ich bin Aura. Wie kann ich helfen?" }]);
  ...
  const sendMsg = () => {
    ...
    setInput("");
    setTimeout(() => setMessages(m => [...m, { role: "bot", text: "Danke! Ich helfe dir gerne weiter." }]), 800);
  };
  ...
  return (
    <div style={{ display: "flex", height: "100vh", fontFamily: "sans-serif", background: "#F0F2F5" }}>
      <aside style={{ width: 248, background: "white", borderRight: "0.5px solid #dce0e6", display: "flex", flexDirection: "column" }}>
        ...
        {navItems.map((item) => (
          <div key={item.name} style={{ padding: "9px 22px", color: item.active ? "#022350" : "#4A4A5A", background: item.active ? "#EEF5FF" : "transparent", borderLeft: item.active ? "2.5px solid #0FA4A0" : "2.5px solid transparent", fontWeight: item.active ? 500 : 400, fontSize: 13, cursor: "pointer" }}>{item.name}</div>
        ))}
        <div style={{ flex: 1 }} />
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <div style={{ background: "white", borderBottom: "0.5px solid #dce0e6", height: 60, padding: "0 32px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: "#9A9AAA" }}>VBV-Frist in 99 Tagen</div>
          </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "24px 28px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, marginBottom: 14 }}>
            {stats.map((s, i) => (
              <div key={i} style={{ background: "white", borderRadius: 14, border: "0.5px solid #dce0e6", padding: "18px 20px" }}>
                <div style={{ fontSize: 34, fontWeight: 500, color: "#022350", lineHeight: 1 }}>{s.n}</div>
                <div style={{ fontSize: 12, color: "#4A4A5A", marginTop: 2 }}>{s.l}</div>
                <div style={{ fontSize: 11, fontWeight: 500, marginTop: 7, color: s.red ? "#e74c3c" : "#0FA4A0" }}>{s.t}</div>
              </div>
            ))}
          </div>
          <div style={{ background: "#022350", borderRadius: 16, padding: "24px 28px", marginBottom: 14, display: "grid", gridTemplateColumns: "1fr auto", gap: 24, alignItems: "center" }}>
            <div>
              <div style={{ fontSize: 10, fontWeight: 600, color: "#C8A24D", marginBottom: 6 }}>VBV-ZERTIFIZIERUNG 2026</div>
              <div style={{ fontSize: 22, fontWeight: 600, color: "white", marginBottom: 12 }}>Dein Lernfortschritt</div>
              ...
              <div style={{ fontSize: 12, color: "rgba(255,255,255,0.5)" }}>342 von 600 Credits</div>
            </div>
            <div style={{ textAlign: "right" }}>
              <div style={{ fontSize: 52, fontWeight: 500, color: "#C8A24D", lineHeight: 1 }}>342</div>
              <div style={{ fontSize: 14, color: "rgba(255,255,255,0.3)" }}>/ 600</div>
            </div>
          </div>
            ...
            {courses.map((c, i) => (
              <div key={i} style={{ borderRadius: 14, overflow: "hidden", position: "relative", minHeight: 200, cursor: "pointer" }}>
                ...
                <div style={{ position: "absolute", inset: 0, padding: "12px 14px 14px", display: "flex", flexDirection: "column", justifyContent: "flex-end", background: "linear-gradient(to top,rgba(2,25,60,0.9) 0%,transparent 50%)" }}>
                  <div style={{ position: "absolute", top: 10, left: 12, background: "rgba(255,255,255,0.15)", color: "white", fontSize: 10, padding: "3px 8px", borderRadius: 20 }}>{c.tag}</div>
                  <div style={{ fontSize: 15, fontWeight: 600, color: "white", marginBottom: 6 }}>{c.title}</div>
                  <div style={{ background: "rgba(255,255,255,0.2)", height: 3, borderRadius: 2, marginBottom: 3 }}>
      ...
      {chatOpen && (
        <div style={{ position: "fixed", top: 0, right: 0, width: 400, height: "100vh", background: "white", borderLeft: "0.5px solid #dce0e6", zIndex: 999999, display: "flex", flexDirection: "column" }}>
          <div style={{ background: "#022350", padding: "18px 20px", display: "flex", alignItems: "center", gap: 12 }}>
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 12, color: "rgba(255,255,255,0.5)" }}>Online</div>
            </div>
            <div onClick={() => setChatOpen(false)} style={{ color: "rgba(255,255,255,0.5)", cursor: "pointer", fontSize: 22 }}>x</div>
          </div>
      ...
      <div onClick={() => setChatOpen(!chatOpen)} style={{ position: "fixed", bottom: 28, right: 28, width: 60, height: 60, borderRadius: "50%", background: "#022350", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 26, cursor: "pointer", zIndex: 99999 }}>robot</div>
    </div>
  );
}